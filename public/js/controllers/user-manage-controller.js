function UserManageController() {
	this.IDENTIFIER = "UserManageController"; // <User>ManageController
	this.DOCUMENT = "user-list.html"; // <user>-list.html file name of view
	this.ViewModel = null;
...
UserManageController.prototype.onReady = function() {
	// Object below should be model of Petrics object being edited
	if (!this.Route.table_name_id) {
		this.Route = new Petrics.SDK.Models.User.ctor();
	}

	this.tableList = new DashboardDatatable('#user-list');
	this.tableList.form = $('#user-edit');
	...
	this.btnNew = $('#new-user-btn');
    this.btnDelete = $('#delete-user-btn');
 ...
 * The method referenced in the following function should be Search<User>s(keyword)
 * It is used to do fast Elastic Search lookup
*/
UserManageController.prototype.Search = function( keyword ) {
	this.ViewModel.SearchUsers(keyword); 
};
...
*/
UserManageController.prototype.BindData = function() {
	var list = this.ViewModel.UserData.items || []; // incase object member is null
...
*/
UserManageController.prototype.OnObjectLoad = function(row, object) {
	// row.find('.item-id').val(object.object_id); // unhide and edit this if you are doing custom calls on row click
	
	row.find('.last_name').text(object.last_name);
	row.find('.first_name').text(object.first_name);
	row.find('.phone').text(object.phone);
...
*/
UserManageController.prototype.OnRowClick = function() {
...
UserManageController.prototype.OnObjectSave = function () {
    var item = {};
    ...
    item = new Petrics.SDK.Models.User.ctor();
...
UserManageController.prototype.DeleteObject = function ( item ) {
...
*/
UserManageController.prototype.ReadyForm = function() {
	var field_name_1 = {};
 ...
 */
UserManageController.prototype.ActivateIndexing = function() {
	if (!this.dataTable) {
