function SupplyManageController() {
	this.IDENTIFIER = "SupplyManageController"; // <Supply>ManageController
	this.DOCUMENT = "supply-list.html"; // <order>-list.html file name of view
	this.ViewModel = null;
...
SupplyManageController.prototype.onReady = function() {
	// Object below should be model of object being edited
	if (!this.Route.id) {
		this.Route = {};
	}

	this.tableList = new DashboardDatatable('#supply-list');
	this.tableList.form = $('#supply-edit');
	...
	this.btnNew = $('#new-supply-btn');
    this.btnDelete = $('#delete-supply-btn');
    ...
    this.ViewModel = new SupplyViewModel(this);
	this.ViewModel.Start();
};

/**
 * Handler for Elastic Search lookup from search box
 * The method referenced in the following function should be Search<Supply>s(keyword)
 * It is used to do fast Elastic Search lookup
*/
SupplyManageController.prototype.Search = function( keyword ) {
	this.ViewModel.SearchSupplys(keyword); 
};
...
*/
SupplyManageController.prototype.BindData = function() {
	var list = this.ViewModel.SupplyData || []; // incase object member is null
...
*/
SupplyManageController.prototype.OnObjectLoad = function(row, object) {
...
SupplyManageController.prototype.OnObjectSave = function () {
    var item = {};
    ...
    if (typeof this.current.id == "undefined") {
        item.id = null;
        result = AccountingApp.SupplyCreateUnSafe(item);
    } else {
        result = AccountingApp.SupplyUpdateUnSafe(item);
    }
...
SupplyManageController.prototype.DeleteObject = function ( item ) {
    result = AccountingApp.SupplyDeleteUnSafe(item.id);
...
*/
SupplyManageController.prototype.ReadyForm = function() {
	var item = {};
 ...
 */
SupplyManageController.prototype.ActivateIndexing = function() {
	if (!this.dataTable) {
